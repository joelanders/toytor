...
class TorServerConnection:
    ...
    async def _server_handshake(self):
        ...
        try:
            self._version = await full_server_handshake(reader, writer, [3], ['127.0.0.1'], '127.0.0.1', self._server.link_cert, self._server.id_cert)
    ...
    async def _run(self):
        ...
        create_cell = await asyncio.wait_for(read_cell(self._reader, self._writer), timeout=1)
        seckey_b = self.bkey
        keys = create.handle_create(self._reader, self._writer, self.fingerprint, seckey_b, create_cell)
    ...
    async def stop(self):
        ...
        if self.run_task:
            ...
            logger.info('canceled run_task in TorServerConnection: %s' % self.run_task)

class TorServer:
    def __init__(self, port):
        self.port = port
        self._server = None
        self.bkey = create.PrivateKey(secret=base64.b64decode(b'99oC9ybcwi6nG6B3kWbRXQVsMWMzp5nsiaGkwvBOSXk='))
        with open('id_key.pem', 'r') as f:
            bites = f.read()
            self.fingerprint = hashlib.sha1(bites.encode()).digest() # check this is right
            self.hexfingerprint = hashlib.sha1(bites.encode()).hexdigest() # (might have whitespace in the file, for example..)
    ...
    def _accept(self, reader, writer):
        conn = TorServerConnection(self, reader, writer, self.bkey, self.fingerprint)
    ...
    async def start(self):
        try:
            ...
            self._server = await asyncio.start_server(self._accept, '127.0.0.1', self.port, ssl=self._context)
